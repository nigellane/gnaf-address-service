...
-- Import the full schema from create-schema.sql
-- States reference table
CREATE TABLE IF NOT EXISTS gnaf.states (
...
CREATE TABLE IF NOT EXISTS gnaf.localities (
...
-- Streets table
CREATE TABLE IF NOT EXISTS gnaf.streets (
...
CREATE TABLE IF NOT EXISTS gnaf.addresses (
    -- Primary identifiers
...
BEGIN
    -- Spatial indexes
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_geometry') THEN
        CREATE INDEX idx_addresses_geometry ON gnaf.addresses USING GIST (geometry);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_localities_geometry') THEN
        CREATE INDEX idx_localities_geometry ON gnaf.localities USING GIST (geometry);
    END IF;
    
    -- Standard indexes
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_locality') THEN
        CREATE INDEX idx_addresses_locality ON gnaf.addresses (locality_pid);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_street') THEN
        CREATE INDEX idx_addresses_street ON gnaf.addresses (street_locality_pid);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_gnaf_pid') THEN
        CREATE INDEX idx_addresses_gnaf_pid ON gnaf.addresses (gnaf_pid);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_status') THEN
        CREATE INDEX idx_addresses_status ON gnaf.addresses (address_status) WHERE address_status = 'CURRENT';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_search_vector') THEN
        CREATE INDEX idx_addresses_search_vector ON gnaf.addresses USING GIN (search_vector);
    END IF;
    
    -- Composite indexes
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_locality_street') THEN
        CREATE INDEX idx_addresses_locality_street ON gnaf.addresses (locality_pid, street_locality_pid);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_coords') THEN
        CREATE INDEX idx_addresses_coords ON gnaf.addresses (latitude, longitude);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_confidence') THEN
        CREATE INDEX idx_addresses_confidence ON gnaf.addresses (confidence_score) WHERE confidence_score >= 80;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_import_batch') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_update_address_search_vector') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_update_address_geometry') THEN
        CREATE TRIGGER trg_update_address_geometry
            BEFORE INSERT OR UPDATE ON gnaf.addresses
            FOR EACH ROW EXECUTE FUNCTION update_address_geometry();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_update_locality_geometry') THEN
...
-- Create views
CREATE OR REPLACE VIEW gnaf.current_addresses AS
SELECT 
    a.*,
    l.locality_name,
    l.postcode,
...
CREATE OR REPLACE VIEW gnaf.high_confidence_addresses AS
SELECT * FROM gnaf.current_addresses
WHERE confidence_score >= 80;

-- Create materialized view for address statistics
DROP MATERIALIZED VIEW IF EXISTS gnaf.address_statistics;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_address_statistics_state') THEN
...
SELECT 'Initial schema with full G-NAF tables created successfully' as status;